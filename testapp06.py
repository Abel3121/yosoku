...
def main():
    ...
    if choice == '学習と検証':
        ...
        if 'df' in st.session_state:
            ...
            # セッションステートに退避していたデータフレームを復元
            df = copy.deepcopy(st.session_state.df)
            gakusyu = ['決定木','ランダムフォレスト','LightGBM','ニューラルネットワーク']
            choice_gakusyu = st.sidebar.selectbox("学習の手法",gakusyu)
            fukasa = st.sidebar.number_input('決定木の深さ(サーバー負荷軽減のためMax=3)',1,3,1)

            # 説明変数と目的変数の設定
            df = pd.get_dummies(df)
            train_Y1 = df["退職_No"]                # 退職列を目的変数にセット
            train_Y2 = df["退職_Yes"]
            train_Y = pd.concat([train_Y1,train_Y2],axis=1)
            train_Y = df['退職_Yes']
            train_X = df.drop("退職_No", axis=1)   # 退職列以外を説明変数にセット
            train_X = train_X.drop("退職_Yes",axis=1)
            
            # train_Y = df["退職_No","退職_Yes"]                # 退職列を目的変数にセット

            # 決定木による予測
            clf, train_pred, train_scores = ml_dtree(train_X, train_Y, fukasa)

            # 正解率を出力
            st.caption('決定木の予測')
            st.header('正解率'+str(round(train_scores,3)))
            st.caption('決定木の可視化')
            ...
            st.caption('AIの予測が「全員退職しない」と偏った場合は(意味がないので)全ての精度は0で表示')
        ...
        st.text('This program was build by Kazuma Ohata who belong to the Department of AIsystem at Wiz')
        st.text('vasion:1.0')    
